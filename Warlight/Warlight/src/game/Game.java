...
public class Game implements Cloneable {
    ...
    public Continent getContinent(int id) {
        return world.getContinent(id);
    }

    public int numRegions() {
        return world.numRegions();
    }
    ...
    public Region getRandomStartingRegion(int forPlayer) {
        for (int pass = 1; pass <= 2; ++pass) {
            ...
            for (Region r : pickableRegions)
                if (regionsOnContinent(r.getContinent(), forPlayer) < 2 && (!bordersEnemy(r, forPlayer) || pass == 2))
                    possible.add(r);
        ...
        for (PlaceArmies move : moves) {
            ...
            if (!isOwnedBy(region, turn))
                ...
                if (armies > left) { // player wants to place more armies than he has left
                    System.out.printf("warning: move wants to place %d armies, but only %d are available\n", armies,
                            left);
    ...
    public static class FightResult {
        ...
        protected void postProcess(int attackingArmies, int defendingArmies) {
            ...
            winner = defendersDestroyed >= defendingArmies ? FightSide.ATTACKER : FightSide.DEFENDER;
        }
    }
